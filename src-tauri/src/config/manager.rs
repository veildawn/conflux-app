        ...
        if !config.rule_providers.is_empty() {
            for (name, provider) in config.rule_providers.iter_mut() {
                ...
                if provider.behavior == "domain" && name_lower.contains("chinadomain") {
                    ...
                    log::info!("Auto-corrected rule provider '{}' behavior back to classical", name);
                    changed = true;
                }
                ...
                if provider.provider_type == "file" {
                    if let Some(path) = &provider.path {
                        ...
                        // 检查路径是否不存在
                        if !path_obj.exists() {
                            let is_linux_path = path.starts_with("/root") || path.starts_with("/home");
                            if is_linux_path {
                                ...
                                if let Some(url) = &provider.url {
                                    if !url.is_empty() {
                                        ...
                                        log::info!("Converted rule provider '{}' to http type", name);
                                        changed = true;
                                        continue;
                                    }
                                }
                                ...
                                if let Ok(data_dir) = crate::utils::get_app_data_dir() {
                                    ...
                                    // 写入空 payload
                                    if let Ok(_) = std::fs::write(&new_path, "payload:\n  - DOMAIN,example.com") {
                                        provider.path = Some(new_path.to_string_lossy().to_string());
