...
import { AlertCircle, ChevronLeft, ChevronRight, Save, Layers, Search, CheckCircle2, Zap, Activity, ShieldAlert, Settings2, Eye, EyeOff, CloudOff } from 'lucide-react';
...
export default function ProxyGroupEditWindow() {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        const groupNames = (config['proxy-groups'] as ProxyGroupConfig[] || []).map((g) => g.name);
        ...
        if (editName) {
           const group = (config['proxy-groups'] as ProxyGroupConfig[] || []).find((g) => g.name === editName);
           if (group) {
             setFormData({
                ...createDefaultFormData(),
                name: group.name || '',
  ...
  const handleSubmit = useCallback(async () => {
        ...
        try {
            ...
            if (needsBehavior) {
              ...
              if (maxFailedTimesValue !== null && !Number.isNaN(maxFailedTimesValue)) group['max-failed-times'] = maxFailedTimesValue;
  ...
  }, [canSubmit, nameValue, typeValue, proxies, providersEnabled, providers, needsBehavior, urlValue, intervalValue, formData, timeoutValue, maxFailedTimesValue, expectedStatusValue, strategyValue, toleranceValue, editName, toast]);
  ...
  const handleNext = useCallback(() => {
    if (step === 'type') {
      if (!nameValue || !!errors.name) {
        toast({ title: '参数错误', description: errors.name || '请选择策略组类型', variant: 'destructive' });
        return;
      }
    }
                ...
                {step === 'type' && (
                  <div className="space-y-8">
                    <div className="space-y-3">
                      <label className="text-xs font-semibold uppercase tracking-widest text-neutral-400">策略组名称</label>
                      <Input
                        value={formData.name}
                        ...
                        className={cn(
                          ...
                          errors.name ? 'border-red-400/60 focus-visible:ring-red-500/20' : 'focus-visible:border-blue-500'
                        )}
                        placeholder="输入名称..."
                        autoFocus
                      />
                    ...
                    <div className="space-y-4">
                      <div className="text-xs font-semibold uppercase tracking-widest text-neutral-400">分发模式</div>
                              ...
                              <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-2">
                                    <h3 className="text-sm font-semibold text-neutral-900">{option.label}</h3>
                                    {option.deprecated && (
                                  ...
                                  <p className="mt-1 text-xs text-neutral-500 leading-relaxed">{option.description}</p>
                                </div>
                                {selected && (
                              ...
                              filteredProxyOptions.map((p) => {
                                ...
                                return (
                                  <div
                                    key={p}
                                    onClick={() =>
                                      setFormData((prev) => ({ ...prev, proxies: toggleListItem(prev.proxies, p) }))
                                    }
                                    className={cn(
                                  ...
                                  >
                                    ...
                                    <span className="text-sm font-semibold text-neutral-800 truncate">{p}</span>
                                  </div>
                                );
                              })
                            )}
                              ...
                              {providerOptions.map((p) => {
                                ...
                                return (
                                  <div
                                    key={p}
                                    onClick={() =>
                                      setFormData((prev) => ({ ...prev, providers: toggleListItem(prev.providers, p) }))
                                    }
                                    className={cn(
                                  ...
                                  >
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <div className="text-xs font-semibold text-neutral-800 truncate">{p}</div>
                                      <div className="text-[10px] text-neutral-400">{isSelected ? '已包含' : '未包含'}</div>
                                    </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="rounded-2xl border border-white/60 bg-white/50 shadow-[0_8px_20px_rgba(0,0,0,0.06)] overflow-hidden">
                            <div
                              ...
                              onClick={() => setFormData(prev => ({...prev, includeAllProxies: !prev.includeAllProxies}))}
                            >
                              <div>
                                <div className="text-sm font-semibold text-neutral-800">包含所有手动代理</div>
                              ...
                              <Switch
                                ...
                                onCheckedChange={(c) => setFormData((prev) => ({ ...prev, includeAllProxies: c }))}
                ...
                {step === 'behavior' && needsBehavior && (
                  <div className="space-y-6">
                    <div className="rounded-3xl border border-white/60 bg-white/55 p-5 shadow-[0_12px_24px_rgba(0,0,0,0.06)] space-y-5">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          <label className="text-xs font-semibold uppercase tracking-widest text-neutral-400">测试间隔 (s)</label>
                          <Input
                        ...
                        <div className="space-y-2">
                          <label className="text-xs font-semibold uppercase tracking-widest text-neutral-400">最大失败次数</label>
                          <Input
                      ...
                      {typeValue === 'url-test' && (
                        <div className="space-y-2">
                          <label className="text-xs font-semibold uppercase tracking-widest text-neutral-400">容差 (ms)</label>
                          <Input
                    ...
                    {typeValue === 'load-balance' && (
                      <div className="space-y-3">
                        <div className="text-xs font-semibold uppercase tracking-widest text-neutral-400">分流算法</div>
                ...
                {step === 'advanced' && (
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      <div className="space-y-4">
                        <div className="text-xs font-semibold uppercase tracking-widest text-neutral-400">排除过滤</div>
                        <div className="rounded-3xl border border-white/60 bg-white/55 p-5 shadow-[0_12px_24px_rgba(0,0,0,0.06)] space-y-4">
                          <div className="space-y-2">
                            <label className="text-xs font-semibold uppercase tracking-widest text-neutral-400">正则排除</label>
                            <Input
                              value={formData.excludeFilter}
                              onChange={(e) => setFormData((prev) => ({ ...prev, excludeFilter: e.target.value }))}
                              className="h-10 rounded-xl bg-white/80 border border-white/70 text-sm font-mono"
                              placeholder="例如: 流量|过期|Back"
                            />
                          </div>
                          <div className="space-y-2">
                            <label className="text-xs font-semibold uppercase tracking-widest text-neutral-400">类型过滤</label>
                            <Input
                      ...
                      <div className="space-y-4">
                        <div className="text-xs font-semibold uppercase tracking-widest text-neutral-400">显示控制</div>
